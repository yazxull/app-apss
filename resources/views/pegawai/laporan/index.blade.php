...
<style>
    ...
    .filter-bar input[type="text"] {
        ...
        background: #FAFAFA; color: var(--text-primary); transition: border-color .2s, box-shadow .2s;
    }
    .filter-bar input[type="text"]:focus {
        outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(124,58,237,0.12); background: #fff;
    }
    .filter-bar select {
    ...
    .table thead th {
        background: #F8FAFC; color: var(--text-secondary); font-weight: 700; font-size: 11px;
    ...
    .table tbody tr:hover td { background: #FAF5FF; }
    ...
    .btn-detail:hover { background: #EDE9FE; color: #1D4ED8; }
    ...
    .pill {
        ...
        transition: all .15s; display: inline-flex; align-items: center; gap: 6px; color: var(--text-secondary);
    }
...
<div class="page-header">
    ...
    <a href="{{ route('pegawai.laporan.create') }}" class="btn btn-primary">
...
<div class="summary-pills mb-3">
    ...
    <a href="{{ route('pegawai.laporan.index', array_merge(request()->query(), ['status' => ''])) }}"
       class="pill {{ !$currentStatus ? 'active' : '' }}">
        Semua <span class="pill-count">{{ $stats['total'] }}</span>
    </a>
    <a href="{{ route('pegawai.laporan.index', array_merge(request()->query(), ['status' => 'menunggu'])) }}"
       class="pill {{ $currentStatus === 'menunggu' ? 'active' : '' }}">
        Menunggu <span class="pill-count">{{ $stats['menunggu'] }}</span>
    </a>
    <a href="{{ route('pegawai.laporan.index', array_merge(request()->query(), ['status' => 'proses'])) }}"
       class="pill {{ $currentStatus === 'proses' ? 'active' : '' }}">
        Diproses <span class="pill-count">{{ $stats['proses'] }}</span>
    </a>
    <a href="{{ route('pegawai.laporan.index', array_merge(request()->query(), ['status' => 'selesai'])) }}"
...
{{-- ===== FILTER BAR ===== --}}
<form method="GET" action="{{ route('pegawai.laporan.index') }}">
    @if(request('status'))
    ...
    <div class="filter-bar">
        ...
        @if(request()->hasAny(['search', 'kategori', 'sort', 'status']))
            <a href="{{ route('pegawai.laporan.index') }}" class="btn-reset">
    ...
    <div class="table-responsive">
        @if ($laporan->isEmpty())
            <div class="empty-state">
                ...
                @if(request()->hasAny(['search', 'kategori', 'status']))
                    <a href="{{ route('pegawai.laporan.index') }}" class="btn btn-primary btn-sm">Reset Filter</a>
                @endif
            </div>
        @else
            <table class="table">
                ...
                <tbody>
                    @foreach ($laporan as $i => $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('pegawai.laporan.show', $item->id) }}" class="btn-action btn-detail">
