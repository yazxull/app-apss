...
<div class="summary-pills mb-3">
    ...
    <a href="{{ route('siswa.laporan.index', array_merge(request()->query(), ['status' => ''])) }}"
...
<form method="GET" action="{{ route('siswa.laporan.index') }}">
    {{-- Pertahankan filter status dari pills --}}
    @if(request('status'))
    ...
    <div class="filter-bar">
        ...
        @if(request()->hasAny(['search', 'kategori', 'sort', 'status']))
            <a href="{{ route('siswa.laporan.index') }}" class="btn-reset">
        ...
        @if ($laporan->isEmpty())
            <div class="empty-state">
                ...
                <p>
                    ...
                    @else
                        Kamu belum pernah membuat laporan pengaduan.
                    @endif
                </p>
                @if(request()->hasAny(['search', 'kategori', 'status']))
                    <a href="{{ route('siswa.laporan.index') }}" class="btn btn-primary btn-sm">
                        Reset Filter
                    </a>
                @else
                    <a href="{{ route('siswa.laporan.create') }}" class="btn btn-primary">
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display:flex;gap:6px;align-items:center;">
                                    <a href="{{ route('siswa.laporan.show', $item->id) }}" class="btn-action btn-detail">
                                        <i class="bi bi-eye"></i> Detail
                                    </a>
                                    @if (!$item->aspirasi || $item->aspirasi->status === 'menunggu')
                                        <form action="{{ route('siswa.laporan.destroy', $item->id) }}"
                                              method="POST"
